...
Token getToken(string Text){
    ...
    while (state != 13) {
        ...
        case 1:	// CHAR STATE
            ...
            if (isBreakChar(Text[index]) || (index == (Text.length())) || isOper(Text[index])) {
...
string getTokens(string input){
    string tokens = "";
    while ((index < (input.length()-1)) || (isOper(input[input.length()-1]) || isChar(input[input.length()-1])) && (index == (input.length()-1))) {
